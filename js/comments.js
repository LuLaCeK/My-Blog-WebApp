// Firebase Realtime Database configuration
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await fetch(`${FIREBASE_DATABASE_URL}/comments.json`, {
                    method: 'POST',
            ...
            commentsArray.forEach(comment => {
                ...
                commentElement.innerHTML = `
                    <div class="comment-header">
                        <span class="author">${escapeHtml(comment.name)}</span>
        ...
        // Start real-time updates with long-polling
        ...
        // Set up event source for real-time updates if the browser supports it
        function setupEventSource() {
            if (typeof EventSource !== 'undefined') {
                const eventSource = new EventSource(`${FIREBASE_DATABASE_URL}/comments.json?auth=${DATABASE_SECRET}`);
        ...
        // Try EventSource first, fall back to polling
        if (!setupEventSource()) {
            startPolling();
        }